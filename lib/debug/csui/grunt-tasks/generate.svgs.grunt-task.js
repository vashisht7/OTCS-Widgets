...
module.exports = function (grunt) {
  ...
  async function convertClassesToInlineStyles(parsedPath, src, dest, suffix, iconStates, sourceStylesByClasses,
    ...
    for (let j = 0; j < Object.keys(iconStates).length; j++) {
      try {
        ...
        // grunt.log.debug(`target styles for state ${state}: ${JSON.stringify(targetStyles)}`);
            ...
            if (node.type === "element") {
              ...
              default:
                if (node.attributes) {
                  ...
                  // grunt.log.debug(`inlineStyles: ${JSON.stringify(inlineStyles)}`);
                  ...
                  //       replaceClassByInlineStyles(child.attributes, inlineStyles);
                  //     }
                  //   }
                  // }
        ...
        // remove the style element before serializing the svg again
        if (svgAst1.children) {
        ...
        // grunt.log.debug(`serialized svg ast: ${outSVGstr}`);
          ...
          transformNode: function (node) {
            ...
            if (node.type === "element" && node.attributes && node.attributes.id) {
              ...
              case 'metaphor':
                delete node.attributes.id;
                if (node.attributes.class) {
                  // if node has the class directly, assume that no children are there
            ...
            if (node.type === "element" && node.name === "style") {
              ...
              for (let k = 0; node.children && k < node.children.length; k++) {
                ...
                // replace colors in classes/selectors
                let styleStr = '';
                const svgClassKeys = Object.keys(svgClasses);
                for (let l = 0; l < svgClassKeys.length; l++) {
                  let cssClass = svgClassKeys[l];

                  // serialize the css class definition and replace styles on the fly
                  // there might be multiple class replacements....
  ...
  /*
    function makeInlineStyles(node) {
      grunt.log.debug(`makeInlineStyles: node: ${JSON.stringify(node)}`);
  ...
  */
        ...
        for (let f = 0; f < children.length; f++) {
          ...
          if (fileInDir !== src && parsedFileInDir.ext === parsedSrc.ext) {
            if (fileInDir.startsWith(nameWithoutExt + '.') ||
              ...
              // grunt.log.debug(`deleting old file: ${fileInDir}`);
  ...
  /*
  * Checks existance of file or directory
  *
  * returns stat structure if file exists
  ...
  */
  ...
  grunt.registerMultiTask('generateSVGs', 'Takes SVG files from a list and generates versions' +
    ...
    try {
      ...
      // grunt.log.debug(`this: ${JSON.stringify(this, undefined, 2)}`);
      ...
      for (let i = 0; i < this.files.length; i++) {
        ...
        // grunt.log.debug(`files ${JSON.stringify(files, undefined, 2)}`);
          ...
          if (oneFileSet.src.length) {
            for (let k = 0; k < oneFileSet.src.length; k++) {
              ...
              for (let l = 0; l < srcFiles.length; l++) {
                ...
                // copy the source file to the dest dir to have all at one place
                // const parsedSrc = path.parse(srcFile);
                // await copyFile(srcFile, path.join(destDir, parsedSrc.base));
              }
            }
          }
        }
