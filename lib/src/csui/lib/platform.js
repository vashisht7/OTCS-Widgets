...
define(['module'
  ...
  function parse(ua) {
    ...
    var objectClass     = 'Object',
        ...
        javaClass       = (isCustomContext && context.java) ? 'JavaPackage' :
    ...
    var version = useFeatures && opera && typeof opera.version == 'function' &&
                  opera.version();
    ...
    function getProduct(guesses) {
      return reduce(guesses, function (result, guess) {
        ...
        if (!result && (result =
                    ...
                    RegExp('\\b' + pattern + '(?:; *(?:[a-z]+[_-])?[a-z]+\\d+|[^ ();-]*)',
                        'i').exec(ua)
            )) {
          // Split by forward slash and append product version if needed.
          if ((result = String(
                  (guess.label && !RegExp(pattern, 'i').test(guess.label)) ? guess.label :
                  result).split('/'))[1] && !/[\d.]+/.test(result[0])) {
            result[0] += ' ' + result[1];
          }
          // Correct character case and cleanup string.
    ...
    // Detect Android browsers.
             ...
             (/\bAndroid\b/.test(os) && /^Chrome/.test(name) &&
              /\bVersion\//i.test(ua))) {
    ...
    if ((data =
                ...
                !/^(?:Trident|EdgeHTML)$/.test(layout) && 'WebKit' ||
                !layout && /\bMSIE\b/i.test(ua) &&
                (os == 'Mac OS' ? 'Tasman' : 'Trident') ||
                layout == 'WebKit' && /\bPlayStation\b(?! Vita\b)/i.test(name) &&
                'NetFront'
        )) {
      layout = [data];
    }
    ...
    if (useFeatures) {
      ...
      if (isHostType(context, 'global')) {
        ...
        if (isModuleScope && isHostType(context, 'system') &&
      ...
      else if (typeof doc.documentMode == 'number' &&
    ...
    if (version && (data =
                ...
                /(?:alpha|beta)(?: ?\d)?/i.exec(ua + ';' +
                                                (useFeatures && nav.appMinorVersion)) ||
    ...
    // Add mobile postfix.
    else if ((/^(?:Chrome|IE|Opera)$/.test(name) || name && !product &&
              !/Browser|Mobi/.test(name)) &&
    ...
    if ((data = (/\bAppleWebKit\/([\d.]+\+?)/i.exec(ua) || 0)[1])) {
      ...
      if (!useFeatures || (!likeChrome && !data[1])) {
        ...
        data = (data = data[0], data < 400 ? 1 : data < 500 ? 2 : data < 526 ? 3 :
                                                                  data < 533 ? 4 :
                                                                  data < 534 ? '4+' :
                                                                  data < 535 ? 5 :
                                                                  data < 537 ? 6 :
        ...
        data = data[1] || (data = data[0], data < 530 ? 1 : data < 532 ? 2 :
                                                            data < 532.05 ? 3 :
                                                            data < 533 ? 4 :
                                                            data < 534.03 ? 5 :
                                                            data < 534.07 ? 6 :
      ...
      layout && (layout[1] += ' ' + (data += typeof data == 'number' ? '.x' :
                                             /[.+]/.test(data) ? '' : '+'));
    ...
    if (product) {
      description.push((/^on /.test(description[description.length - 1]) ? '' : 'on ') +
                       product);
    }
    ...
    if (os) {
      ...
      os = {
        ...
        'toString': function () {
          ...
          return this.family + ((version && !isSpecialCasedOS) ? ' ' + version : '') +
                 (this.architecture == 64 ? ' 64-bit' : '');
        }
      };
    }
    // Add browser/OS architecture.
    if ((data = /\b(?:AMD|IA|Win|WOW|x86_|x)64\b/i.exec(arch)) &&
        !/\bi686\b/i.test(arch)) {
      if (os) {
        os.architecture = 64;
