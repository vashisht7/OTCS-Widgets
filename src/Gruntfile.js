...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    requirejsBundleIndex: {
      all: {
        src: [
          'bundles/otcss-all.js'
        ],
        dest: 'bundles/otcss-index',
        options: {
          prefix: 'otcss'
        }
      }
    ...
    replace: {
      ...
      debug: {
        files: [
          {
            src: ['otcss-extensions.json'],
            dest: '../out-debug/otcss-extensions.json'
          }
        ]
      },
      release: {
        files: [
          {
            src: ['otcss-extensions.json'],
            dest: '../out-release/otcss-extensions.json'
          }
        ]
      }
    },
    ...
    copy: {
      module: {
        files: [
          {
            cwd: "../srcmodules/otcssamples",
            src: "**",
            dest: "../out-module/otcssamples",
            expand: true
          },
          {
            cwd: "../out-release",
            src: "**",
            dest: "../out-module/otcssamples/support",
            expand: true
          }
        ]
      }
    ...
    languagepack: {
      all: {
        options: {
          ...
          bundleIndexes: {
            'bundles/otcss-index': ['bundles/otcss-all']
          },
